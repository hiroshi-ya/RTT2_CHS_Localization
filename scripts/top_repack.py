# Thie script rebuild the top level .DAT/.FAT
# Author: HiroshiYa
...
def main():
    ...
    # DATNewFile = FATName + "_NEW/" + "CMN" + ".DAT"
    DATNewFile = "../outputs/CMN.DAT"
    ...
    while True:
        ...
        readPathMod  = outPath + fileNameExt + "/MOD/" + fileName   # modded file's path
        if fileNameExt == "FAT":                                    # if a FAT file, check for NEW version
            newName = fileName.split('.')[0] + "_NEW.FAT"
            newPath = outPath + "FAT/" + newName # ../CMN/CMN/FAT/newName_NEW.FAT
            folderPath = outPath + "FAT/" + fileName.split('.')[0] # ../CMN/CMN/FAT/newName
            if os.path.exists(newPath): # only write NEW file if its folder or DAT exists
                if os.path.exists(folderPath):
                    print("** " + newName + " detected **")
                    readPath = newPath
                elif os.path.exists(outPath + "DAT/mod/" + fileName.split('.')[0] + ".DAT"): 
                    print("** " + newName + " and its DAT detected **")
                    readPath = newPath
                else:
                    print("\n** Warning: A new FAT detected but cannot locate its content")
                    print("** Ignoring the new FAT\n")

        elif fileNameExt == "GIM":
            # newPath = outPath + "FAT/general_pic/" + fileName
            generalPicPath = "../sources/general_pic/" + fileName
            if os.path.exists(generalPicPath):
                print("** MOD " + fileName + " detected in general_pic**")
                readPath = generalPicPath

        if os.path.exists(readPathMod):
            print("** MOD " + fileName + " detected **")
            if fileNameExt == "DAT": # special treatment for DATs under the DAT folder
                if os.path.exists(outPath + "FAT/" + fileName.split('.')[0] + "_NEW.FAT"): # check if its modified FAT is in the right place
                    print("** Detected " + fileName + "\'s new FAT in the FAT folder")
                    readPath = readPathMod
                else: # if the FAT doesn't exist, don't change the readPath
                    print("\n** Warning: A modified DAT detected but cannot locate its FAT")
                    print("** Ignoring the new DAT\n")
            else:
                readPath = readPathMod
    ...
    # FATNewFile = FATName + "_NEW/" + "CMN" + ".FAT"
    FATNewFile = "../outputs/CMN.FAT"
